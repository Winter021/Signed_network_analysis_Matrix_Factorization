...
#   random.shuffle(list(zip(data_points))) #shuffle data points into random order
#   data_points = (list(data_points))#debug
#   fold_size = math.ceil(len(data_points)/num_folds)
...
#       folds.append(data_points[int(fold_size*fold_index):])
#     else:
#       folds.append(data_points[int(fold_size*fold_index): int( fold_size*(fold_index + 1) ) ])
#   return folds
